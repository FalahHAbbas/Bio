...
public class UsbDeviceDataExchangeImpl {
  ...
  public boolean DataExchange(byte[] out_data, byte[] in_data, int in_time_out, int out_time_out,
      boolean keep_open, boolean use_max_end_point_size, int trace_level) {
    synchronized (this) {
      ...
      try {
        ...
        if (out_data.length > 0) {
          transfer_bytes = usb_ctx.mDevConnetion_
              .bulkTransfer(usb_ctx.mWritePoint_, out_data, out_data.length, out_time_out);
        ...
        while (to_read_size >= getTransferBuffer().length) {
          transfer_bytes = usb_ctx.mDevConnetion_
              .bulkTransfer(usb_ctx.mReadPoint_, getTransferBuffer(), getTransferBuffer().length,
                  in_time_out);
          ...
          if (copy_pos + transfer_bytes > in_data.length) {
            Log.e(log_tag, String.format("Small receive buffer. Need %d bytes",
                copy_pos + transfer_bytes - in_data.length));
            return res;
          }
        ...
        while (to_read_size >= transfer_buffer_size_2) {
          transfer_bytes = usb_ctx.mDevConnetion_
              .bulkTransfer(usb_ctx.mReadPoint_, getTransferBuffer(), transfer_buffer_size_2,
                  in_time_out);
        ...
        if (to_read_size > usb_ctx.mReadPoint_.getMaxPacketSize()) {
          ...
          if (data_left > 0) {
            transfer_bytes = usb_ctx.mDevConnetion_
                .bulkTransfer(usb_ctx.mReadPoint_, getTransferBuffer(), data_left, in_time_out);
        ...
        while (to_read_size > 0) {
          transfer_bytes = usb_ctx.mDevConnetion_
              .bulkTransfer(usb_ctx.mReadPoint_, getTransferBuffer(),
                  use_max_end_point_size ? usb_ctx.mReadPoint_.getMaxPacketSize() : to_read_size,
                  in_time_out);
          ...
          int real_read =
              to_read_size > usb_ctx.mReadPoint_.getMaxPacketSize() ? usb_ctx.mReadPoint_
                  .getMaxPacketSize() : to_read_size;

          if (copy_pos + real_read > in_data.length) {
            Log.e(log_tag, String.format("Small receive buffer. Need %d bytes",
                copy_pos + real_read - in_data.length));
            return res;
          }
